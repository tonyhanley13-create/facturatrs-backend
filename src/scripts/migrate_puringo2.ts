...
async function migratePuringo2() {
  ...
  try {
    ...
    console.log('🚀 Iniciando migración ultra-rápida de puringo2 -> FacturaTRS...');
    ...
    if (companyCheck.recordset.length > 0) {
      ...
      const createComp = await pool.request().query(`
        ...
        (nombre, rnc, direccion, telefono, email, modalidad_facturacion, proximo_numero_factura, id_usuario, fecha_creacion, fecha_actualizacion)
        OUTPUT INSERTED.id
        VALUES 
        ('Ooptions Xpress', '131976077', 'Santo Domingo, Rep. Dom.', '809-566-5080', 'info@optionsxpress.com', 'tradicional', '4124', 20, GETDATE(), GETDATE())
      `);
      companyId = createComp.recordset[0].id;
      console.log(`✨ Creada nueva empresa 'Ooptions Xpress' con ID: ${companyId}`);
    }

    // Asegurar que el super admin (usuario 20) tenga acceso a esta empresa
    ...
    for (const cl of oldClients.recordset) {
      ...
      const ins = await pool.request().query(`
        ...
        (id_empresa, nombre, rnc, telefono, direccion, tipo_cliente, id_usuario, fecha_creacion)
        OUTPUT INSERTED.id
        VALUES 
        (${companyId}, '${name.replace(/'/g, "''")}', '${rnc}', '${phone.replace(/'/g, "''")}', '${address.replace(/'/g, "''")}', 'individual', 20, '${created}')
      `);
    ...
    for (const m of oldMercs.recordset) {
      ...
      const ins = await pool.request().query(`
        ...
        (id_empresa, tipo, nombre, descripcion, precio, porcentaje_itbis, codigo_item, estado, id_usuario, fecha_creacion, fecha_actualizacion)
        OUTPUT INSERTED.id
        VALUES 
        (${companyId}, 'servicio', '${name.replace(/'/g, "''")}', '${name.replace(/'/g, "''")}', ${price}, 18, '${code.replace(/'/g, "''")}', 'activo', 20, '${created}', GETDATE())
      `);
    ...
    const controlMap = new Map<string, number>(); // Key: control string -> FacturaTRS invoice id
    ...
    for (const inv of oldInvoices.recordset) {
      ...
      const ins = await pool.request().query(`
        ...
        (id_empresa, id_cliente, id_usuario, numero_factura, ncf, tipo_documento, subtotal, monto_itbis, monto_descuento, monto_flete, monto_otros, monto_total, moneda, estado, descripcion, fecha_creacion, fecha_actualizacion)
        OUTPUT INSERTED.id
        VALUES 
        (${companyId}, ${clieStr}, 20, '${invNumber}', ${ncfStr}, 'tradicional', ${subtotal}, ${tax}, 0, 0, 0, ${total}, 'DOP', 'issued', '${comment.replace(/'/g, "''")}', '${created}', GETDATE())
      `);
    ...
    for (const item of oldItems.recordset) {
      ...
      await pool.request().query(`
        ...
        (id_factura, id_producto, numero_linea, codigo_item, nombre_item, descripcion, cantidad, precio_unitario, porcentaje_descuento, porcentaje_itbis, subtotal, monto_itbis, monto_total, fecha_creacion)
        VALUES 
        (${invId}, ${prodStr}, 1, '${mercCode.replace(/'/g, "''")}', '${description.replace(/'/g, "''")}', '${description.replace(/'/g, "''")}', ${qty}, ${price}, 0, 18, ${subtotal}, ${itemTax}, ${total}, '${created}')
      `);
      itemInsertCount++;
    }
    ...
    // Sincronizar tabla secuencias_ncf si existe
    try {
      await pool.request().query(`
    ...
    console.log('\n🎉 ¡MIGRACIÓN DE puringo2 A Ooptions Xpress COMPLETADA CON ÉXITO!');
