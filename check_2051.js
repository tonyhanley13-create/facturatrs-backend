...
(async () => {
  ...
  if (!company?.certificate_content) { console.log('No cert'); return; }
  ...
  for (const envName of ['TesteCF', 'CerteCF', 'eCF']) {
    try {
      ...
      console.log(`${envName}:`, JSON.stringify(result));
    } catch (e) {
      console.log(`${envName}: Error - ${e.message}`);
    }
  }
